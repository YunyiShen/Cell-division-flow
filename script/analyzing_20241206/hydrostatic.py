...
def concentration_rot(x,y,t, scale_short = 0.1, scale_long = 0.4, scale_t = 0.1, t0 = -0.01, rotate = (83/90) * np.pi/2):
    local_t = max( t - t0, 0)

    rot_mat = np.array([[np.cos(rotate), -np.sin(rotate)],
                        [np.sin(rotate), np.cos(rotate)]])
    Sigma = np.array([[1/scale_short**2, 0],
                      [0, 1/scale_long**2]])
    rot_mat = rot_mat @ Sigma @ rot_mat.T
    xy = np.stack([x, y], axis = 2) 
    #breakpoint()
    xyrot = np.einsum('...i,ij->...j', xy, rot_mat) 
    #breakpoint()
    xy = np.sum(xyrot * xy, axis = 2)
    #breakpoint()
    gau = 1 - ( min((local_t)/scale_t,1)) * np.exp(-.5  * xy)
    gau *= (gau >= 0)
    return gau
...
stress_inner_save = \
            naivesolver_with_extra_hydrostatic(nt = nt, u = u, v = v, 
               dt = dt, dx = dx, 
               dy = dy, mask = mask,
                X = X, Y = Y, hydro_stress = concentration_rot,
                upwind = True,
               rho = 1, nu = 0.6, save_every = 2, save_from =50, #150,
...
axs[1, 0].set_xlabel('')
axs[1, 0].set_ylabel('')
axs[1, 0].arrow(.6, 0.6, -0.4, -0.4, head_width=0.2, head_length=0.2, fc='black', ec='black')
