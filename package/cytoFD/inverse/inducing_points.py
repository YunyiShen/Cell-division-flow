...
class InducingPointKernel2(Kernel):
    def __init__(
        self,
        base_kernel: Kernel,
        ...
        super(InducingPointKernel2, self).__init__(active_dims=active_dims)
        ...
        self.n1, self.d = inducing_points.shape[-2:]
        self.output_idxs = (torch.arange(self.n1).unsqueeze(1)*(self.d) + (torch.tensor(self.out_dims).unsqueeze(0) )).reshape(len(self.out_dims)*self.n1)  
    ...
    def _inducing_mat(self):
        if not self.training and hasattr(self, "_cached_kernel_mat"):
            ...
            res = to_dense(self.base_kernel(self.inducing_points, self.inducing_points)[..., self.output_idxs, :][..., :, self.output_idxs])
    ...
    def _get_covariance(self, x1, x2):
        ...
        if torch.equal(x1, x2):
            ...
            if not self.training and settings.sgpr_diagonal_correction.on():
                correction = (self.base_kernel(x1, x2, diag=True)[...,pi1] - covar.diagonal(dim1=-1, dim2=-2)).clamp(0, math.inf)
                covar = LowRankRootAddedDiagLinearOperator(covar, DiagLinearOperator(correction))
        else:
            k_ux2 = to_dense(self.base_kernel(x2, self.inducing_points)[..., pi2, :][..., :, self.output_idxs])
    ...
    def _covar_diag(self, inputs):
        ...
        covar_diag = to_dense(self.base_kernel(inputs, diag=True)[..., pi1])
